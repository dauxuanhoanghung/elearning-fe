    ...
    >
      <header className="mb-2 flex items-center justify-between">
        <div className="flex flex-wrap items-center">
          <Link className="mr-3 inline-flex items-center text-sm font-semibold text-gray-900 dark:text-white">
            <Avatar
              src="https://flowbite.com/docs/images/people/profile-picture-2.jpg"
              isSignalShown={false}
            />
            {user?.firstName || "AAAAD"}
          </Link>
      ...
      <div className="mt-4 flex items-center space-x-4">
        <button className="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400">
          {true ? <FavoriteFullIcon /> : <FavoriteIcon />}
          24 {t("comment.like")}
        </button>
        {!isEmptyObject(currentUser) && (
